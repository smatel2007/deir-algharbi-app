...
def init_db():import flet as ft
import urllib.parse
from datetime import datetime
from supabase import create_client, Client

# --- إعدادات الاتصال السحابي (Supabase) ---
SUPABASE_URL = "https://vmggwpxbyfbeflypeend.supabase.co"
SUPABASE_KEY = "sb_secret_EXzGUnwzLleWPx2Qzg_Iqw_hUV-LJdJ"
...
def main(page: ft.Page):
    ...
    # دالة موحدة وآمنة لفتح الروابط الخارجية (واتساب، اتصال هاتفي، إلخ)
    ...
    if page.current_user is None:
        saved_uname = db_select("session_store", {"key": "saved_username"})
        saved_n = db_select("session_store", {"key": "saved_name"})
        if saved_uname and saved_n:
            page.current_user = {"username": saved_uname[0]["value"], "name": saved_n[0]["value"]}
            if saved_uname[0]["value"] not in page.online_users:
                page.online_users.append(saved_uname[0]["value"])
    ...
    def show_main_page(e=None):
        ...
        def logout_action(e):
            if page.current_user:
                ...
                db_delete("session_store", {"key": "saved_username"})
                db_delete("session_store", {"key": "saved_name"})
                page.current_user = None
            show_main_page()
            ...
            if user_data:
                ...
                if db_pass == password:
                    if status == 'approved':
                        ...
                        db_insert("session_store", {"key": "saved_username", "value": username})
                        db_insert("session_store", {"key": "saved_name", "value": name})
    ...
    def show_public_chat_room():
        ...
        def clear_public_chat_db(e):
            db_delete("public_chats", {})
            show_message("🧹 تم مسح سجل الدردشة العامة بالكامل!")
            show_public_chat_room()

        clear_btn = ft.IconButton(icon=ft.Icons.DELETE_SWEEP, icon_color="white", tooltip="مسح كل الرسائل", on_click=clear_public_chat_db)
        ...
        def delete_public_msg(msg_id):
            db_delete("public_chats", {"id": msg_id})
            show_message("🗑️ تم حذف الرسالة بنجاح")
            show_public_chat_room()

        db_history = db_select("public_chats", order_by="id", ascending=True)
        for msg in db_history:
            mid, sender, username, text, time = msg["id"], msg["sender"], msg["username"], msg["text"], msg["time"]
            ...
            if is_me or page.current_user["username"] == "khaled":
                delete_btn = ft.IconButton(
                    ...
                    tooltip="حذف هذه الرسالة", on_click=lambda _, m_id=mid: delete_public_msg(m_id)
                )
    ...
    def show_private_chat_room(target_username, target_name):
        ...
        def clear_private_chat(e):
            page.private_chats[chat_id] = []
            show_message("🧹 تم مسح المحادثة الخاصة!")
            show_private_chat_room(target_username, target_name)
    ...
    def show_admin_panel(stats_mode="all"):
        ...
        for ann in db_anns:
            aid, text, time = ann["id"], ann["text"], ann["time"]
