...
# --- إعداد مسار التخزين المحلي لبيانات الجلسة ---
...
def init_cloud_data():
    try:
        ...
        if not shops:
            db_insert("shops", {"name": "سوبرماركت البركة", "type": "مواد غذائية", "description": "توفر كافة المواد الغذائية والخضروات الطازجة مع عروض أسبوعية مميزة.", "owner": "المبرمج خالد", "status": "approved"})
            db_insert("shops", {"name": "ألبسة النخبة", "type": "ملابس وأزياء", "description": "تشكيلة واسعة من الألبسة الرجالية والنسائية بأفضل الأسعار.", "owner": "أحمد العلي", "status": "approved"})
...
def main(page: ft.Page):
    ...
    # --- نظام سجل التنقل (Navigation Stack) للتحكم بزر الرجوع ---
    if not hasattr(page, "nav_history"):
        page.nav_history = []

    def navigate_to(page_func, *args, **kwargs):
        if hasattr(page, "current_page_func") and page.current_page_func:
            if page.current_page_func != page_func:
                page.nav_history.append((page.current_page_func, page.current_args, page.current_kwargs))
        
        page.current_page_func = page_func
        page.current_args = args
        page.current_kwargs = kwargs
        page_func(*args, **kwargs)

    def go_back(e=None):
        if page.nav_history:
            prev_func, p_args, p_kwargs = page.nav_history.pop()
            page.current_page_func = prev_func
            page.current_args = p_args
            page.current_kwargs = p_kwargs
            prev_func(*p_args, **p_kwargs)
        else:
            show_message("أنت في الصفحة الرئيسية بالفعل")
    ...
    def background_global_sync():
        while not global_stop_event.is_set():
            time.sleep(120)
            try:
                if getattr(page, "current_view", "") in ["main", "shops", "municipality", "phone_book"]:
    ...
    def build_custom_bottom_bar(active_index):
        def make_nav_btn(icon_name, label_text, target_index, target_func):
            is_active = (target_index == active_index)
            return ft.GestureDetector(
                on_tap=lambda _: navigate_to(target_func) if not is_active else None,
        ...
        return ft.Container(
            content=ft.Row([
                make_nav_btn(ft.Icons.HOME, "الرئيسية", 0, show_main_page),
                make_nav_btn(ft.Icons.STOREFRONT, "الدليل", 1, show_shops_page),
                make_nav_btn(ft.Icons.CHAT, "الدردشة", 2, show_chats_dashboard),
                make_nav_btn(ft.Icons.PHONE, "الهواتف", 3, show_phone_book_page),
    ...
    def build_header(title_text, on_back_click=None, bg_color="bluegrey800", trailing_action=None):
        ...
        back_action = on_back_click if on_back_click else lambda _: go_back()
        stack_controls.append(
            ft.Container(
                content=ft.IconButton(
                    icon=ft.Icons.ARROW_BACK, icon_color="white", icon_size=24, on_click=back_action
                ),
    ...
    def create_shop_card(shop_name, shop_type, shop_desc, shop_owner):
        type_colors = {
        ...
        if shop_desc and shop_desc.strip():
            card_columns.extend([
                ft.Divider(height=10, color="grey200"),
                ft.Text(f"📝 {shop_desc}", size=13, color="grey800")
            ])
        ...
        return ft.Container(
            content=ft.Column(card_columns, spacing=4),
    ...
    def show_main_page(e=None):
        ...
        if page.current_user:
            ...
            account_button = ft.TextButton(
                ...
                on_click=lambda _: navigate_to(show_login_page)
            )
        ...
        btn_municipality = create_button("🏛️ بلدية دير الغربي", "bluegrey800", lambda _: navigate_to(show_municipality_page))
        btn_shops = create_button("🛍️ دليل المحلات التجارية", "orange800", lambda _: navigate_to(show_shops_page)) 
        btn_chats = create_button("💬 غرف ومحادثات الدردشة", "bluegrey700", lambda _: navigate_to(show_chats_dashboard))
        btn_phone_book = create_button("📞 دليل الهواتف والوظائف", "blue", lambda _: navigate_to(show_phone_book_page)) 

        footer = ft.Container(
            content=ft.Column([
                ft.Text("تم التطوير بواسطة خالدالأحمد[cite: 1]", size=12, color="grey700"),
                ft.TextButton(content=ft.Text("🔐 لوحة التحكم للمشرف", color="bluegrey400", size=11), on_click=lambda _: navigate_to(show_admin_login_page))
    ...
    def show_municipality_page():
        ...
        mun_header = build_header("بلدية دير الغربي", bg_color="bluegrey800")

        def show_sub_service(service_key, title_name, icon_widget):
            page.clean()
            sub_header = build_header(title_name, bg_color="green800")
        ...
        btn_layout = create_button("🗺️ مخطط البلدة", "teal700", lambda _: navigate_to(show_sub_service, "layout", "مخطط البلدة", ft.Icons.MAP))
        btn_sectors = create_button("📐 المقاسم", "brown700", lambda _: navigate_to(show_sub_service, "sectors", "المقاسم", ft.Icons.LANDSCAPE))
        btn_cleaning = create_button("🧹 خدمات النظافة", "bluegrey600", lambda _: navigate_to(show_sub_service, "cleaning", "خدمات النظافة", ft.Icons.CLEANING_SERVICES))
        btn_water = create_button("💧 ضخ المياه", "blue700", lambda _: navigate_to(show_sub_service, "water", "ضخ المياه", ft.Icons.WATER_DROP))
    ...
    def show_login_page(e=None):
        ...
        login_header = build_header("تسجيل دخول الأعضاء", bg_color="bluegrey800")
        ...
        btn_go_signup = ft.TextButton("ليس لديك حساب؟ تقدم بطلب تسجيل جديد", on_click=lambda _: navigate_to(show_signup_page))
    ...
    def show_signup_page():
        ...
        signup_header = build_header("طلب تسجيل حساب جديد", bg_color="bluegrey800")
        ...
        def process_signup(e):
            ...
            if check_exists:
                ...
                navigate_to(show_waiting_approval_page)
    ...
    def show_waiting_approval_page():
        ...
        scrollable_content = ft.ListView([
            ...
            ft.Container(content=ft.Column([
                ...
                create_button("↩️ العودة للرئيسية كـ زائر", "green", lambda _: navigate_to(show_main_page))
            ], horizontal_alignment=ft.CrossAxisAlignment.CENTER))
        ], expand=True)
        page.add(
            ft.Column([
    ...
    def show_chats_dashboard():
        ...
        chat_header = build_header("قسم الدردشة والتواصل", bg_color="bluegrey700")
        ...
        if not page.current_user:
            scrollable_content = ft.ListView([
                ...
                ft.Container(content=ft.Column([
                    ...
                    create_button("🔐 تسجيل الدخول الآن", "green", lambda _: navigate_to(show_login_page)),
                ], horizontal_alignment=ft.CrossAxisAlignment.CENTER))
            ], expand=True)
            page.add(
                ft.Column([
        ...
        btn_public_room = create_button("👥 غرفة الدردشة العامة للمجتمع", "bluegrey800", lambda _: navigate_to(show_public_chat_room))
                ...
                members_list_view.controls.append(
                    ft.Container(
                        content=ft.ListTile(
                            ...
                            on_click=lambda _, tu=u_username, tn=u_name: navigate_to(show_private_chat_room, tu, tn)
                        ),
    ...
    def show_public_chat_room():
        ...
        room_header = build_header("الدردشة العامة للبلدة", bg_color="bluegrey800", trailing_action=clear_btn)
    ...
    def show_private_chat_room(target_username, target_name):
        ...
        def clear_private_chat(e):
            db_history = db_select("private_chats", {"chat_id": chat_id})
            for msg in db_history:
                db_delete("private_chats", {"id": msg["id"]})
            show_message("🧹 تم مسح السجل الخاص من قاعدة البيانات!")
            refresh_private_ui()

        clear_btn = ft.IconButton(icon=ft.Icons.DELETE_SWEEP, icon_color="white", tooltip="مسح السجل", on_click=clear_private_chat)
        room_header = build_header(f"محادثة: {target_name}", bg_color="bluegrey900", trailing_action=clear_btn)
        
        p_messages_column = ft.Column(spacing=8)

        def delete_private_msg(msg_id):
            db_delete("private_chats", {"id": msg_id})
            show_message("🗑️ تم حذف الرسالة الخاصة")
            refresh_private_ui()

        def load_private_messages():
            p_messages_column.controls.clear()
            db_history = db_select("private_chats", {"chat_id": chat_id}, order_by="id", ascending=True)
            for msg in db_history:
                mid = msg["id"]
                ...
                is_me = (sender_username == page.current_user["username"])
                
                delete_btn = ft.Container()
                if is_me:
                    delete_btn = ft.IconButton(
                        icon=ft.Icons.DELETE_OUTLINE, icon_size=14, icon_color="white70",
                        tooltip="حذف الرسالة", on_click=lambda _, m_id=mid: delete_private_msg(m_id)
                    )
                ...
                p_messages_column.controls.append(
                    ft.Row([
                        ft.Container(
                            content=ft.Column([
                                ft.Row([
                                    ft.Text(text, size=14, color="white" if is_me else "black", expand=True),
                                    delete_btn
                                ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                                ft.Text(time_str, size=9, color="white60" if is_me else "grey500")
                            ], spacing=2),
        ...
        stop_polling_private = threading.Event()
        def poll_private_messages():
            while not stop_polling_private.is_set():
                time.sleep(2)
                if page.current_view != "private_chat":
                    break
                try:
                    load_private_messages()
                    page.update()
        ...
        t_priv = threading.Thread(target=poll_private_messages, daemon=True)
        t_priv.start()

        def on_view_pop_private(e):
            stop_polling_private.set()

        page.on_view_pop = on_view_pop_private
    ...
    def show_add_adv_page():
        page.clean()
        page.current_view = "add_adv"
        adv_header = build_header("إضافة إعلان تجاري معتمد", bg_color="bluegrey700")
        ...
        txt_shop_desc = ft.TextField(label="وصف المنتجات، الأصناف أو العروض المتاحة", multiline=True, height=80)
        ...
        def submit_adv(e):
            ...
            db_insert("shops", {
                ...
                "description": txt_shop_desc.value.strip() if txt_shop_desc.value else "",
                "owner": txt_shop_owner.value,
                "status": "pending"
            })
            show_message("✅ تم إرسال إعلانك بنجاح بانتظار موافقة المشرف!")
            navigate_to(show_shops_page)

        btn_submit = create_button("🚀 إرسال المنشور للمشرف", "green", submit_adv)
        
        scrollable_content = ft.ListView([
            adv_header, ft.Container(height=15),
            ft.Container(content=ft.Column([txt_shop_name, ft.Container(height=5), txt_shop_type, ft.Container(height=5), txt_shop_desc, ft.Container(height=5), txt_shop_owner, ft.Container(height=15), btn_submit], horizontal_alignment=ft.CrossAxisAlignment.CENTER))
        ], expand=True)
    ...
    def show_shops_page():
        ...
        shops_header = build_header("دليل المحلات التجارية", bg_color="orange800")
        
        if not page.current_user:
            btn_add_adv = create_button("🔒 سجل دخول لإضافة إعلانك مجاناً", "bluegrey400", lambda _: navigate_to(show_login_page))
        else:
            btn_add_adv = create_button("➕ إضافة إعلانك التجاري مجاناً", "bluegrey700", lambda _: navigate_to(show_add_adv_page))
            
        search_field = ft.TextField(label="ابحث عن محل أو تصنيف أو منتج...", border_color="orange800", on_change=lambda e: filter_shops(e.control.value))
        shops_container = ft.Column(horizontal_alignment=ft.CrossAxisAlignment.CENTER)
        
        def filter_shops(t):
            shops_container.controls.clear()
            active_shops = db_select("shops")
            for shop in active_shops:
                status_val = str(shop.get("status", "")).strip().lower()
                if status_val not in ["approved", ""]:
                    continue
                name, stype, desc, owner = shop["name"], shop["type"], shop.get("description", ""), shop["owner"]
                search_query = t.lower()
                if search_query in name.lower() or search_query in stype.lower() or search_query in desc.lower(): 
                    shops_container.controls.append(create_shop_card(name, stype, desc, owner))
            page.update()
    ...
    def show_phone_book_page():
        page.clean()
        page.current_view = "phone_book"
        phone_header = build_header("دليل الهواتف والوظائف", bg_color="blue")
    ...
    def show_admin_login_page():
        ...
        admin_header = build_header("لوحة دخول المشرف", bg_color="bluegrey900")
        txt_password = ft.TextField(label="أدخل كلمة مرور الإدارة السرية", password=True)
        
        def check_login(e):
            if txt_password.value == ADMIN_PASSWORD: navigate_to(show_admin_panel)
    ...
    def open_edit_dialog(title, fields_dict, on_save_callback):
        page.clean()
        edit_header = build_header(title, bg_color="bluegrey700")
        controls_list, inputs_refs = [], {}
        for key, value in fields_dict.items():
            is_multiline = ("وصف" in key or "تفاصيل" in key)
            tf = ft.TextField(label=key, value=value, multiline=is_multiline, height=80 if is_multiline else None)
            inputs_refs[key] = tf
    ...
    def show_admin_panel(stats_mode="all"):
        ...
        admin_header = build_header("إدارة تطبيق الدير الغربي", bg_color="bluegrey900")
        ...
        stats_row = ft.Row([
            ft.GestureDetector(on_tap=lambda _: navigate_to(show_admin_panel, "subscribers"), content=ft.Container(content=ft.Column([ft.Text("المشتركين", size=12), ft.Text(str(total_users), size=20, weight="bold")], horizontal_alignment="center"), bgcolor="blue50", padding=10, border_radius=8, expand=True)),
            ft.GestureDetector(on_tap=lambda _: navigate_to(show_admin_panel, "online"), content=ft.Container(content=ft.Column([ft.Text("المتصلين", size=12), ft.Text(str(len(page.online_users)), size=20, weight="bold")], horizontal_alignment="center"), bgcolor="green50", padding=10, border_radius=8, expand=True))
        ])
        ...
        def edit_mun_service(s_key, s_title, current_content):
            navigate_to(open_edit_dialog, f"تعديل {s_title}", {"تفاصيل القسم / المواعيد": current_content},
        ...
        # --- جلب الإعلانات المعلقة بشكل مرن ---
        shop_pending_container = ft.Column(horizontal_alignment=ft.CrossAxisAlignment.CENTER)
        all_shops_for_check = db_select("shops")
        pending_s = [s for s in all_shops_for_check if str(s.get("status", "")).strip().lower() in ["pending", ""]]
        
        for s in pending_s:
            sid, name, stype, desc, owner = s["id"], s["name"], s["type"], s.get("description", ""), s["owner"]
            shop_pending_container.controls.append(
                ft.Container(content=ft.Column([
                    ft.Text(f"🏪 إعلان معلق: {name} ({stype})", weight="bold"),
                    ft.Text(f"📝 الوصف: {desc if desc else 'بدون وصف'}", size=11, color="grey700"),
                    ft.Row([
        ...
        approved_s = [s for s in all_shops_for_check if str(s.get("status", "")).strip().lower() == "approved"]
        
        def edit_shop(sid, current_name, current_type, current_desc, current_owner):
            navigate_to(open_edit_dialog, "تعديل بيانات المحل", {
                "اسم المحل": current_name, 
                "تصنيف المحل": current_type, 
                "وصف المنتجات والعروض": current_desc,
                "إدارة المحل": current_owner
            }, lambda vals: db_update("shops", {
                "name": vals["اسم المحل"], 
                "type": vals["تصنيف المحل"], 
                "description": vals["وصف المنتجات والعروض"],
                "owner": vals["إدارة المحل"]
            }, {"id": sid}))
            ...
            active_shops_container.controls.append(
                ft.Container(content=ft.Row([
                    ft.Column([
                        ...
                        ft.Text(f"تصنيف: {stype} | 👤 {owner}", size=10, color="grey600"),
                        ft.Text(f"📝 {desc}", size=11, color="grey800", max_lines=1, overflow="ellipsis") if desc else ft.Container()
                    ], expand=True),
                    ft.IconButton(ft.Icons.EDIT, icon_color="orange800", on_click=lambda _, idx=sid, n=name, t=stype, d=desc, o=owner: edit_shop(idx, n, t, d, o)),
        ...
        def edit_phone(pid, current_name, current_phone):
            navigate_to(open_edit_dialog, "تعديل جهة الإتصال", {"المسمى الوظيفي": current_name, "رقم الهاتف": current_phone},
                        lambda vals: db_update("phone_book", {"name": vals["المسمى الوظيفي"], "phone": vals["رقم الهاتف"]}, {"id": pid}))
                             
        def add_new_phone(e):
            navigate_to(open_edit_dialog, "إضافة رقم جديد", {"اسم الجهة": "", "الرقم": ""},
                        lambda vals: db_insert("phone_book", {"name": vals["اسم الجهة"].strip(), "phone": vals["رقم الهاتف"].strip()}) if vals["اسم الجهة"].strip() else None)
        ...
        btn_show_all = ft.TextButton("🔄 عرض كافة أقسام الإدارة", icon=ft.Icons.RESTART_ALT, on_click=lambda _: navigate_to(show_admin_panel, "all")) if stats_mode != "all" else ft.Container()
