...
def main(page: ft.Page):
    ...
    if not hasattr(page, "local_hidden_public_msgs"):
        page.local_hidden_public_msgs = set() # لتخزين معرفات الرسائل المحذوفة من الهاتف المحلي فقط
    ...
    # --- خيط تحديث البرنامج العام (كل دقيقتين)[cite: 1] ---
    global_stop_event = threading.Event()
    def background_global_sync():
        while not global_stop_event.is_set():
            time.sleep(120)  # كل دقيقتين[cite: 1]
            try:
    ...
    def show_public_chat_room():
        ...
        def clear_public_chat_local_phone(e):
            # مسح السجل من الهاتف المحلي فقط عبر إضافة كافة الرسائل الحالية لقائمة الإخفاء الخاصة بالهاتف
        ...
        def delete_public_msg_for_everyone(msg_id):
            # حذف الرسالة نهائياً من السحابة لتختفي لدى الجميع
        ...
        def load_public_messages():
            ...
            for msg in db_history:
                ...
                # إذا كانت الرسالة محذوفة محلياً من هذا الهاتف فقط، يتم تخطي عرضها
                ...
                # يمكن للمرسل أو المشرف حذف الرسالة لدى الجميع
        ...
        # مؤقت زمني لجلب الرسائل تلقائياً كل ثانيتين (2 ثانية كما طلبت)[cite: 1]
        stop_polling = threading.Event()
        def poll_messages():
            while not stop_polling.is_set():
                time.sleep(2)  # كل ثانيتين[cite: 1]
